...
def dump_api_token_scopes():
    api_scopes = data.get('components').get('schemas').get('ApiToken').get('properties').get('scopes').get('items').get('enum')
    for api_scope in sorted(api_scopes):
        print(api_scope)
