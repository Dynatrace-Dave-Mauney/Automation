    ...
    for settings_object in settings_object_list:
        ...
        for item in items:
            ...
            if not summary_mode:
                ...
                rows.append([config_item_title, enabled, scope])

            count_total += 1

    summary.append(f'There are {count_total} log ingest rules currently defined.')

    summary = sorted(summary)

    if not summary_mode:
        rows = sorted(rows)
        report_name = 'Log Ingest Rules'
        report_writer.initialize_text_file(None)
        report_headers = ['Name', 'Enabled', 'Scope']
