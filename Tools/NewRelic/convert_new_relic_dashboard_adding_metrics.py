...
new_relic_data = []
# dynatrace_dashboard_template = None
id_prefix = 'faaaaaaa-faaa-faaa-fbbb-'
# tile_metrics_dictionary = None
...
def process_new_relic_dashboards():
    ...
    # global tile_metrics_dictionary
    # print(tile_metrics_dictionary)
...
def get_dashboard_template():
    ...
    dashboard_template_file_name = 'dynatrace_dashboard_usql_single_value_template.json'
...
def generate_dynatrace_dashboard():
    ...
    # dashboard = json.loads(copy.deepcopy(dynatrace_dashboard_template))
    dynatrace_dashboard_template = get_dashboard_template()
    dashboard = copy.deepcopy(dynatrace_dashboard_template)
    tile_metrics_dictionary = load_tile_metrics()
    print(dashboard)
    tiles = dashboard.get('tiles')
    # template_header = tiles[0]
    template_usql_single_value_tile = tiles[1]
    # print(template_header)
    # print(template_usql_single_value_tile)
    filename = None
    last_filename = None
    id_number = 1
    width = 950
        ...
        for filename, new_relic_dashboard_name, page_name, widget_title, widget_visualization_id, widget_query, threshold_alert_severity, threshold_value in [inner_list]:
            if filename != last_filename:
                ...
                if last_filename is not None:
                    ...
                    id_number += 1
                dashboard_id = id_prefix + format_id_number(id_number)
                dashboard['id'] = dashboard_id
                dashboard_metadata = dashboard.get('dashboardMetadata')
                print(dashboard_id)
                print(dashboard_metadata)
                dashboard_metadata['name'] = new_relic_dashboard_name
                dashboard['tiles'] = []
                last_filename = filename
            usql_single_value_tile = copy.deepcopy(template_usql_single_value_tile)
            name = widget_title
            short_name = name
            name += ': ' + widget_query
            print(tile_metrics_dictionary)
            usql_query = tile_metrics_dictionary[page_name][widget_title]['USQL']
            usql_single_value_tile['name'] = name
            usql_single_value_tile['customName'] = short_name
            usql_single_value_tile['query'] = usql_query
            ...
            # usql_single_value_tile['visualConfig']['type'] = dynatrace_vizualization
            usql_single_value_tile['bounds']['top'] = top
            usql_single_value_tile['bounds']['left'] = left
            dashboard['tiles'].append(usql_single_value_tile)
            if top >= 4712:
                print('Too many tiles...skipping this tile')
            else:
                # if left >= (5 * width):  # Allow 6 Tiles per row
                if left >= width:  # Allow 2 Tiles per row
                    top += height
                    left = 0
                else:
                    left += width
    print(f'Writing {dashboard.get("id")} to {filename}')
...
def load_tile_metrics():
    path = 'convert_new_relic_dashboard_adding_metrics.xlsx'
    wb_obj = openpyxl.load_workbook(path)
    sheet_obj = wb_obj.active

    tile_metrics_dictionary = {}

    m_row = sheet_obj.max_row
    for i in range(2, m_row + 1):
        new_relic_page_name_cell = sheet_obj.cell(row=i, column=3)
        new_relic_widget_title_cell = sheet_obj.cell(row=i, column=4)
        new_relic_query_cell = sheet_obj.cell(row=i, column=6)
        dynatrace_usql_query_cell = sheet_obj.cell(row=i, column=9)
        new_relic_page_name = new_relic_page_name_cell.value
        new_relic_widget_title = new_relic_widget_title_cell.value
        new_relic_query = new_relic_query_cell.value
        dynatrace_usql_query = dynatrace_usql_query_cell.value

        # print(new_relic_page_name, new_relic_widget_title, new_relic_query, dynatrace_usql_query)

        if not tile_metrics_dictionary.get(new_relic_page_name, None):
            tile_metrics_dictionary[new_relic_page_name] = {}

        if new_relic_query:
            tile_metrics_dictionary[new_relic_page_name][new_relic_widget_title] = {"NRQL": new_relic_query, "USQL": dynatrace_usql_query}

    return tile_metrics_dictionary
