...
def process():
	...
	path = 'MSHS Server Inventory TEMP.xlsx'
	wb_obj = openpyxl.load_workbook(path)
	sheet_obj = wb_obj.active

	m_row = sheet_obj.max_row
	for i in range(2, m_row + 1):
		host_cell = sheet_obj.cell(row=i, column=3)
		env_cell = sheet_obj.cell(row=i, column=8)
		app_cell = sheet_obj.cell(row=i, column=6)
		ip_address_cell = sheet_obj.cell(row=i, column=4)
		os_cell = sheet_obj.cell(row=i, column=5)
		function_cell = sheet_obj.cell(row=i, column=7)
		zone_cell = sheet_obj.cell(row=i, column=10)
		winrm_cell = sheet_obj.cell(row=i, column=13)
		tier_cell = sheet_obj.cell(row=i, column=11)
		batch_cell = sheet_obj.cell(row=i, column=15)
		deployment_date_cell = sheet_obj.cell(row=i, column=16)
		oneagent_install_status_cell = sheet_obj.cell(row=i, column=2)
		oneagent_uninstall_status_cell = sheet_obj.cell(row=i, column=17)
		notes_cell = sheet_obj.cell(row=i, column=18)
	...
	report_writer.write_xlsx('MSHS Server Inventory Automation TEMP.xlsx', 'Automation', report_headers, rows, header_format=None, auto_filter=(0, len(report_headers) - 1))
