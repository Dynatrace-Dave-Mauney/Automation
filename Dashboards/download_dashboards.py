	...
	try:
		...
		for entry in res['dashboards']:
			...
			if dashboard_owner == 'Dynatrace':
				print(dashboard_owner, dashboard_id, dashboard_name)
				tr = requests.get(env + '/api/config/v1/dashboards/' + dashboard_id, headers=headers)
				# save(path, dashboard_id, tr.json())
				save(path, dashboard_name + '.json', tr.json())
	except ssl.SSLError:
		print("SSL Error")
