...
def process(env, token):
	...
	for entry in res['dashboards']:
		...
		if 'AWS' in dashboard_name or 'Amazon' in dashboard_name:
			if dashboard_owner == 'Dynatrace':
				dashboard_links_format = f"		('{dashboard_name}', '#dashboard;id={dashboard_id}'),"
				lines.append(dashboard_links_format)
				# lines.append(f'{dashboard_name}|{dashboard_id}')
